...
class DataSource:
    ...
    def get_use_rows_by_demographic(self, demographic):
        ...
        try: 
            cursor = self.connection.cursor()
            cursor.execute("SELECT birthcontrol_use FROM test WHERE states=%s OR region=%s OR homeownership=%s OR marriage=%s OR employ1=%s OR education=%s OR race=%s OR poliParty=%s OR poliView=%s OR religion=%s OR insurance=%s", (demographic, demographic, demographic, demographic, demographic,  demographic, demographic, demographic, demographic, demographic, demographic,))
